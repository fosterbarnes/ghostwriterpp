...
$exePath = Join-Path $repoRoot "$buildDirName/bin/ghostwriter.exe"
...
try {
    if (-not $NoBuild) {
        & $buildScript -Preset $Preset -Clean:$Clean -CraftRoot $CraftRoot -StrictExeIcon:$StrictExeIcon
    }
    ...
    Write-Host "ghostwriter.exe exited with code $exit (0x$([Convert]::ToString($exit, 16)))"
    if ($exit -ne 0) {
        exit $exit
    }
}
