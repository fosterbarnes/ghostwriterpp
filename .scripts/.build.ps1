...
Another process may have CMakeCache.txt or the build tree open (IDE, CMake GUI, antivirus). Close it or reboot, then retry. You can also delete ``$Path`` manually.
Underlying error: $($lastError.Exception.Message)
"@
}

function Repair-StaleCMakeCacheIfNeeded {
    param(
        [Parameter(Mandatory)] [string]$BuildDirPath,
    ...
    foreach ($line in [System.IO.File]::ReadLines($cacheFile)) {
        if ($line.StartsWith("CMAKE_HOME_DIRECTORY:")) {
            $idx = $line.IndexOf("=")
            if ($idx -ge 0) {
                $cachedRaw = $line.Substring($idx + 1).Trim()
            }
            break
        }
    }
    ...
    if ([string]::Equals($cached, $expected, [StringComparison]::OrdinalIgnoreCase)) {
        return
    }

    Write-Warning @"
CMake cache was generated for a different source directory:
  cache: $cached
  build: $expected
Removing CMakeCache.txt and CMakeFiles under $BuildDirPath.
"@
    Remove-PathWithRetry -Path $cacheFile
    $cmakeFiles = Join-Path $BuildDirPath "CMakeFiles"
    if (Test-Path -LiteralPath $cmakeFiles) {
        Remove-PathWithRetry -Path $cmakeFiles -Recurse
    }
}
...
try {
    ...
    if (-not $Clean) {
        Repair-StaleCMakeCacheIfNeeded -BuildDirPath $buildDirPath -ExpectedSourceRoot $repoRoot
    }
    if ($Clean) {
    ...
    $builtExe = Join-Path $buildDirPath "bin/$ExeFileName"
